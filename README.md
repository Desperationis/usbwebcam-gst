...
## `playWebcam.bash`
Plays a gst pipe that streams video from the usb-camera streaming to /dev/video0 to a UDP port, 8081.

To run this script, simply input the ip of the DESTINATION computer: `bash playWebcam.bash {target's ip}`

## `receiveJpeg.bash`
Decodes M-JPEG encoded stream from UDP socket and displays it. Run this script on the target computer like so: `bash receiveJpeg.bash`. With this, it will automatically check if port 8081 is having video streamed to it
